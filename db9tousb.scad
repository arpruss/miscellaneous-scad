...
topWall = 1.5;
bottomWall = 2;
bottomScrewLength = 6; // 8;
DB9Width = 17.82;
DB9ScrewSpacing = 25;
DB9ScrewDiameter = 3;
DB9Height = 9.13;
DB9MinimumZToCutout = 6;
screwDiameter = 1.8;
...
bottomHeight = max(bottomWall+bottomOffset+pcbThickness,bottomWall+DB9Height+DB9MinimumZToCutout);
...
stm32ScrewY1 = (fatPillarLocations[0][1]+fatPillarLocations[1][1])/2-stm32ScrewYSpacing/2;
...
module DB9() {
    module DB9ScrewHole() {
        rotate([0,90,0]) cylinder(d=DB9ScrewDiameter+2*tolerance,$fn=16,h=sideWall+2*nudge);
    }
    
   translate([innerLength-nudge,innerWidth/2-DB9Width/2-tolerance,bottomHeight-DB9Height]) cube([sideWall+2*nudge,DB9Width+tolerance,DB9Height+nudge]);
   translate([innerLength-nudge,innerWidth/2-DB9ScrewSpacing/2,bottomHeight-DB9Height/2]) DB9ScrewHole();
   translate([innerLength-nudge,innerWidth/2+DB9ScrewSpacing/2,bottomHeight-DB9Height/2]) DB9ScrewHole();
}
...
module bottom() {
    ...
    difference() {
       ...
       if (includeNunchuckPort)
        ...
        translate([-sideWall-nudge,(stm32ScrewY1+stm32ScrewYSpacing/2)-usbPortWidth/2,bottomWall+bottomOffset-usbPortZOffsetDown]) cube([sideWall+2*nudge,usbPortWidth,usbPortHeight+nudge]);
       if (includeGamecubePort) 
