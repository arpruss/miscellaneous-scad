...
module sideHole(z1) {
    ...
    rotate([0,0,90])
    translate([0,maxDiameter/2,0]) rotate([90,0,0])
...
difference() {
    ...
    cylinder(d1=scaleByZ(wallThickness,bottomDiameter)-2*wallThickness,d2=scaleByZ(height-wallThickness,bottomDiameter)-2*wallThickness,h=height-wallThickness);
